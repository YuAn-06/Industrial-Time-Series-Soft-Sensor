...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x_enc):
        ...
        y_pred_back = self.weight_B @ h + self.bias_B
        y_pred_front = self.weight_F @ h + self.bias_F
...
class Model(nn.Module):
    ...
    def soft_sensor(self, x_enc):
        x_input = x_enc[:, :, :-self.C_out]  # [B, T, 12]
        y_input = x_enc[:, :, -self.C_out:]  # [B, T, 1]
        y_input_res = y_input
        y_pred_list = []

        for attention_block in self.attention_block_list:
            block_input = torch.cat((x_input, y_input_res), dim=-1)  # [B, T, 13]
            y_pred_back, y_pred_front = attention_block(block_input)
            y_pred_list.append(y_pred_front)
            y_input_res = y_input - y_pred_back[:, :, -self.C_out:]

        y_pred_list = torch.stack(y_pred_list, dim=-1)  # [B, pred_len, C_out, E]
        y_pred = torch.sum(y_pred_list, dim=-1)  # [B, pred_len, C_out]

        return y_pred[:, 0, :]  # pred_len=1 -> [B, C_out]
