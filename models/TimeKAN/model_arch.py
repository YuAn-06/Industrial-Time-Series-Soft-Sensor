...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.normalize_layers = torch.nn.ModuleList(
            [
                StandardNorm(self.configs.enc_in, affine=True, non_norm=True if configs.use_norm == 0 else False)
    ...
    def short_term_forecasting(self, x_enc):
        ...
        dec_out = self.predict_layer(dec_out.permute(0, 2, 1)).permute(
                0, 2, 1)  
        dec_out = self.projection_layer(dec_out).reshape(B, self.configs.C_in, self.pred_len).permute(0, 2, 1).contiguous()
    ...
    def soft_sensor(self, x_enc):
        ...
        # dec_out = self.predict_layer(dec_out.permute(0, 2, 1)).permute(
        #         0, 2, 1)
        dec_out = dec_out[:,-1, :] # [B* C, d_model]
        # dec_out = self.projection_layer(dec_out).reshape(B, self.configs.C_in, self.pred_len).permute(0, 2, 1).contiguous()
        dec_out = self.projection_layer(dec_out).reshape(B, self.configs.C_in, 1).contiguous() # [B, C, 1]
        dec_out = self.predict_layer(dec_out.squeeze(-1))
        return dec_out
